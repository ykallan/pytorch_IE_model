    ...
    def train(self, config : Config, device):
        ...
        for epoch in range(config.epoch):
            ...
            if f1 >= best_f1:
                ...
                if config.from_pertrained not in ['bert', 'albert']:
                    torch.save(embedding.state_dict(), '{}/{}_sp_o_embedding.pkl'.format(base_path, config.from_pertrained))
                torch.save(sp_model.state_dict(), '{}/{}_sp_model.pkl'.format(base_path, config.from_pertrained))
                torch.save(o_model.state_dict(), '{}/{}_o_model.pkl'.format(base_path, config.from_pertrained))
            else:
                f1_not_up_count += 1
...
def load_model_and_evalute(config: Config, device):
    ...
    embedding.load_state_dict(torch.load('{}/{}_sp_o_embedding.pkl'.format(base_path, config.from_pertrained), map_location='cuda:0'))
    sp_model.load_state_dict(torch.load('{}/{}_sp_model.pkl'.format(base_path, config.from_pertrained), map_location='cuda:0'))
    o_model.load_state_dict(torch.load('{}/{}_o_model.pkl'.format(base_path, config.from_pertrained), map_location='cuda:0'))
