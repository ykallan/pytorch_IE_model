...
class SpoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sp_start = [[0] * self.num_predicate for _ in range(max_len)]
        sp_end = [[0] * self.num_predicate for _ in range(max_len)]
        o_start = [0] * max_len
        o_end = [0] * max_len

        for spo in spo_list:
            predicate_id = preidcate2id[spo['predicate']]
            sp_start[spo['subject_start']][predicate_id] = 1
            sp_end[spo['subject_end'] - 1][predicate_id] = 1
...
class SubjectPredicateModel(nn.Module):
    def __init__(self, embedding_size: int, num_predicate: int, num_heads: int, rnn_type: str, rnn_hidden_size: int, forward_dim: int, device: str='cuda', dropout_prob: float=0.1):
        ...
        self.num_predicate = num_predicate
        ...
        self.sp_start_fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(forward_dim, num_predicate * 2),
        )
    ...
    def forward(self, input_embedding: Tensor, mask: Tensor, position_embedding: Tensor):
        ...
        # 输出要reshape
        batch_size, seq_len = sp_start.shape[0], sp_start.shape[1]

        sp_start = sp_start.reshape(batch_size, seq_len, self.num_predicate, 2)
        sp_end = sp_end.reshape(batch_size, seq_len, self.num_predicate, 2)
        ...
        for epoch in range(config.epoch):
            ...
            with tqdm(total=steps) as pbar:
                for step, inputs_outputs in enumerate(train_data_loader):
                    ...
                    sp_start_true = inputs_outputs['sp_start'].long().to(device)
                    sp_end_true = inputs_outputs['sp_end'].long().to(device)
                    o_start_true = inputs_outputs['o_start'].long().to(device)
                    o_end_true = inputs_outputs['o_end'].long().to(device)
                    ...
                    sp_start_pred = sp_start_pred.permute(0, 3, 1, 2)
                    sp_end_pred = sp_end_pred.permute(0, 3, 1, 2)
                    ...
                    sp_start_loss =torch.masked_select(ce_loss(sp_start_pred, sp_start_true), sp_loss_mask)
                    sp_end_loss = torch.masked_select(ce_loss(sp_end_pred, sp_end_true), sp_loss_mask)
                    ...
                    # 计算总的损失
                    loss_sum = 2.0 * (sp_start_loss + sp_end_loss) + (o_start_loss + o_end_loss)
            ...
            if f1 >= best_f1:
                ...
                if config.from_pertrained not in ['bert', 'albert']:
                    torch.save(embedding.state_dict(), '{}/{}_sp_o_embedding_ce.22.pth'.format(base_path, model_name))
                torch.save(sp_model.state_dict(), '{}/{}_sp_model_ce.22.pth'.format(base_path, model_name))
                torch.save(o_model.state_dict(), '{}/{}_o_model_ce.22.pth'.format(base_path, model_name))
            else:
                f1_not_up_count += 1
...
def compute_batch_sp(sp_model: SubjectPredicateModel, embeddings: tuple, text: list, config: Config):
    ...
    sp_start_pred = sp_start_pred.cpu().detach().numpy()
    sp_end_pred = sp_end_pred.cpu().detach().numpy()
...
def compute_o(o_model: ObjectModel, embeddings: tuple, share_feature: Tensor, share_mask: Tensor, o_query_text: list, config: Config):
    ...
    o_start_pred = o_start_pred.cpu().detach().numpy()
    o_end_pred = o_end_pred.cpu().detach().numpy()
...
def load_model_and_test(config: Config, device):
    ...
    embedding.load_state_dict(torch.load('{}/{}_sp_o_embedding_ce.22.pth'.format(base_path, model_name), map_location='cuda:0'))
    sp_model.load_state_dict(torch.load('{}/{}_sp_model_ce.22.pth'.format(base_path, model_name), map_location='cuda:0'))
    o_model.load_state_dict(torch.load('{}/{}_o_model_ce.22.pth'.format(base_path, model_name), map_location='cuda:0'))
