    ...
    def train(self, config: Config, device):
        ...
        for epoch in range(config.epoch):
            ...
            if f1 >= best_f1:
                ...
                if config.from_pertrained not in ['bert', 'albert']:
                    torch.save(embedding.state_dict(), '{}/{}_sp_o_embedding_ce.pkl'.format(base_path, model_name))
                torch.save(sp_model.state_dict(), '{}/{}_sp_model_ce.pkl'.format(base_path, model_name))
                torch.save(o_model.state_dict(), '{}/{}_o_model_ce.pkl'.format(base_path, model_name))
            else:
                f1_not_up_count += 1
